...
def inicial_phrase(wheasel):
    # Creates a inicial completely string random with the
    # same lenght of the target phrase.
...
def apply_mutation(father):
    ...
    # Recieves the phrase (the "father") and
    # returns a new version with random mutations,
    ...
    for character in father:
        if random.randint(1, 100) <= mutation_rate:
            ...
            # replaces the character for a random.
...
def assessment_of_phrase(phrase):
    ...
    # Compares a sentence with the target_phrase and
    # returns a similarity score (fitness).
...
def execute_simulation():
    ...
    # Main function that orchestrates and
    # executes the evolutionary cycle of the Doninha Program.
    ...
    print(f"G{generation_counter}: {close_position}/{len(target_phrase)}")
    print(f"Phrase: '{close_phrase}'")
    # Print the initial generation with limited line length
    print(
        "Generation {}: Position Numbers={}/{}".format(
            generation_counter, close_position, len(target_phrase)
        )
    )
    print("Phrase='{}'".format(close_phrase))
    # Main loop
    while close_position < len(target_phrase):
        ...
        print(f"Position Number: {close_position}/{len(target_phrase)}")
        print(f"Phrase: '{close_phrase}'")
    print("\n=== SIMULATION COMPLETE! ===")

# =========ENTRANCE OF THE PROGRAM=========
# Ensures that the function execute_simulation()
# is called when the file is executed.
